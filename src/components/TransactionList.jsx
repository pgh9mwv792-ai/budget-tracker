      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm divide-y divide-slate-100 dark:divide-slate-800">
        ...
        {filtered.map((t) =>
          editingId === t.id ? (
            ...
            <div key={t.id}>
              <div className="flex items-center justify-between px-4 py-3 text-sm hover:bg-slate-50 dark:hover:bg-slate-800/50 transition">
                <div className="flex items-center gap-4 min-w-0">
                  ...
                  {receiptFor(t.id) && (
                    <button
                      onClick={() => setOpenReceiptFor((cur) => (cur === t.id ? null : t.id))}
                      title="View itemized receipt"
                      className="text-xs bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 rounded px-1.5 py-0.5 shrink-0 hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition"
                    >
                      🧾 receipt
                    </button>
                  )}
              ...
              {openReceiptFor === t.id && receiptFor(t.id) && (
                <ReceiptDetail receipt={receiptFor(t.id)} />
              )}
            </div>
          )
        )}
